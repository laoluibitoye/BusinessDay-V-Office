...
$drStmt = $db->prepare("SELECT id, name, email, role FROM users WHERE line_manager_id=? AND is_active=1");
$drStmt->execute([$user['id']]); $directReports = $drStmt->fetchAll(); $drIds = array_column($directReports,'id');

$pendingLeaveList = [];
if (!empty($drIds)) {
    $ph = implode(',', array_fill(0, count($drIds), '?'));
    $lS = $db->prepare("SELECT lr.*,u.name as rname FROM leave_requests lr JOIN users u ON u.id=lr.user_id WHERE lr.user_id IN ($ph) AND lr.current_stage IN ('submitted','lm_review') ORDER BY lr.created_at DESC LIMIT 6");
...
$overdueTeam=array_filter($teamTasks,function($t){return $t['due_date']&&strtotime($t['due_date'])<strtotime(date('Y-m-d'));});
...
?>
<style>
.hri-page{padding:20px;max-width:1100px;margin:0 auto;}
.wlcbar{background:linear-gradient(135deg,#db2777 0%,#9d174d 100%);border-radius:12px;padding:18px 22px;margin-bottom:18px;display:flex;align-items:center;gap:14px;box-shadow:0 8px 32px rgba(219,39,119,.2);}
...
.kpi.nv{border-top-color:#002850;}.kpi.gn{border-top-color:#64A014;}.kpi.rd{border-top-color:#dc2626;}.kpi.wn{border-top-color:#f59e0b;}.kpi.pk{border-top-color:#db2777;}
...
.gmain{display:grid;grid-template-columns:1fr 280px;gap:16px;}.cleft,.cright{display:flex;flex-direction:column;gap:16px;}
...
.annrow{padding:10px 14px;border-bottom:1px solid #f8fafc;}.annrow:last-child{border-bottom:none;}.antit{font-size:13px;font-weight:600;color:#0f172a;}.anbod{font-size:12px;color:#64748b;margin-top:3px;}
...
<div class="wlcbar">
    ...
    <div><div class="wname"><?=$greeting?>, <?=$firstName?> <?=$role['icon']?></div><div class="wsub"><?=date('l, d F Y')?> &bull; <?=htmlspecialchars($role['label'])?> &bull; <?=count($directReports)?> direct reports</div></div>
...
<div class="krow">
    ...
    <a class="kpi <?=count($pendingLeaveList)>0?'pk':'nv'?>" href="leave-approvals.php"><div class="kval" data-kpi="dr_leave_pending"><?=count($pendingLeaveList)?></div><div class="klbl">Team Leave</div><div class="ksub">pending</div></a>
    <a class="kpi <?=count($teamTasks)>0&&!empty($overdueTeam)?'rd':'nv'?>" href="tasks.php"><div class="kval"><?=count($teamTasks)?></div><div class="klbl">Team Tasks</div><div class="ksub"><?=!empty($overdueTeam)?count($overdueTeam).' overdue':'open'?></div></a>
...
<div class="gmain">
    <div class="cleft">
        ...
        <div class="card">
            <div class="chd"><div class="cht">&#9989; Team Tasks</div><a class="chl" href="tasks.php">All tasks &#8594;</a></div>
            <?php if(empty($teamTasks)): ?><div class="empty">No open team tasks</div>
            <?php else: $pc3=['urgent'=>'#dc2626','high'=>'#f59e0b','normal'=>'#002850','low'=>'#94a3b8'];
            ...
            ?>
            <div class="ritem">
                <div class="tdot" style="background:<?=$pc3[$t['priority']]??'#94a3b8'?>"></div>
                <div style="flex:1;"><div style="font-size:13px;"><?=htmlspecialchars(substr($t['title'],0,45))?></div><div style="font-size:11.5px;color:#94a3b8;"><?=htmlspecialchars($t['uname'])?>  &bull; <?=ucfirst($t['priority'])?></div></div>
    ...
    <div class="cright">
        ...
        <div class="card">
            <div class="chd"><div class="cht">&#127958; My Leave Balance <?=date('Y')?></div></div>
        ...
        <div class="card">
            <div class="chd"><div class="cht">&#128101; My Team (<?=count($directReports)?>)</div></div>
            <?php if(empty($directReports)): ?><div class="empty">No direct reports</div>
            <?php else: foreach($directReports as $dr): $rl=ROLES[$dr['role']]['label']??$dr['role']; ?>
            <div class="ritem">
                <div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:600;"><?=htmlspecialchars($dr['name'])?></div><div style="font-size:11.5px;color:#94a3b8;"><?=$rl?></div></div>
